...
def run_one(
    ...
    hp = build_run_hp(dataset=dataset, runtime_s=runtime_s, seed=seed, base_net=base_net, stale_tau=stale_tau)
...
def main():
    ...
    ap.add_argument("--runtime-s", type=int, default=1500)
    ...
    ap.add_argument("--results-root", type=str, default="ablation_results_stale")
    ...
    base_net = {
        ...
        "latency_per_km": 5.0,
        "base_loss_ms": 0.02,
        "loss_per_100m": 0.03,  # 0.03, 0.045, 0.06
        "out_of_range": 900.0,
        "chunk_size_kb": 4,
    }

    results_root = ensure_dir(Path(args.results_root)) / f"stale_tau_{str(10.0)}"
                ...
                for seed in seeds:
                    ...
                    try:
                        metrics = run_one(
                            ...
                            base_net=base_net,
                            stale_tau=10.0 # 1.0, 2.0, 5.0, 10.0
                        )
